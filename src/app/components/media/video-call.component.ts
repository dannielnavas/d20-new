...
@Component({
  ...
  template: `
    <div class="flex flex-col items-center gap-4 transition-all duration-500 relative w-full group">
      
      <!-- Controles de Video/Audio y Conexión Flotantes -->
      <div class="flex items-center gap-2 bg-black/50 p-1.5 rounded-2xl border border-white/10 backdrop-blur-md shadow-xl transition-all duration-300 hover:bg-black/60 pointer-events-auto">
        @if (isCallActive()) {
          <button
            class="relative flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-300 cursor-pointer"
            [ngClass]="peerService.isAudioMuted() ? 'bg-rose-500/20 text-rose-400 hover:bg-rose-500/30' : 'bg-white/5 text-slate-200 hover:bg-white/10 hover:text-white'"
          ...
          <button
            class="relative flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-300 cursor-pointer"
            [ngClass]="peerService.isVideoMuted() ? 'bg-rose-500/20 text-rose-400 hover:bg-rose-500/30' : 'bg-white/5 text-slate-200 hover:bg-white/10 hover:text-white'"
      ...
      @if (isCallActive() || peerService.localStream() || remoteStreamEntries().length > 0) {
        <div class="flex gap-4 overflow-x-auto pb-4 pt-2 snap-x snap-mandatory scrollbar-none scroll-smooth px-2 items-center justify-center w-full relative pointer-events-auto">
          
          <!-- Stream local -->
          @if (peerService.localStream()) {
            <div class="relative w-28 h-28 shrink-0 bg-slate-900 rounded-full overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.6)] transition-all duration-300 snap-center group border-4 ring-2 ring-black/50" 
          ...
          @for (entry of remoteStreamEntries(); track entry.key) {
            <div class="relative w-28 h-28 shrink-0 bg-slate-900 rounded-full overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.6)] border-4 border-slate-700/50 ring-2 ring-black/50 snap-center group hover:border-emerald-500/40 transition-all duration-500 hover:scale-105 hover:shadow-[0_0_20px_rgba(16,185,129,0.2)]">
      ...
      @if (peerService.error()) {
        <div class="absolute -top-16 bg-rose-500/90 border border-rose-400 text-white px-4 py-2 rounded-xl text-sm flex items-center gap-2 shadow-2xl animate-in fade-in slide-in-from-bottom-4 pointer-events-auto z-50">
