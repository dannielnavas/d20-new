      ...
      @for (roll of rolls(); track roll.id) {
        <div class="dice-roll-toast flex flex-col items-center">
          <div class="text-white text-lg md:text-xl font-bold drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] mb-4 bg-slate-900/90 px-6 py-2 rounded-full backdrop-blur-md border border-slate-600/50 flex items-center gap-2 shadow-xl flex-wrap justify-center text-center">
            ...
            @if (roll.rolls.length > 1 && roll.mode === 'normal') {
              <span class="text-emerald-300 font-mono text-sm ml-1 bg-slate-950/50 px-2.5 py-0.5 rounded-md border border-emerald-500/20">
                ({{ roll.rolls.join(' + ') }})
              </span>
            }
            @if (roll.mode === 'advantage') {
              ...
              <span class="text-slate-400 font-mono text-xs ml-1 bg-slate-950/50 px-2 py-0.5 rounded-md">({{ roll.rolls[0] }} y {{ roll.rolls[1] }})</span>
            }
          ...
          <div class="relative flex flex-col items-center gap-6 group">
            ...
            @if (roll.rolls.length > 1 && roll.mode === 'normal') {
              <div class="flex items-center justify-center gap-3 flex-wrap max-w-lg md:max-w-xl">
                @for (subRoll of roll.rolls; track $index) {
                  <div class="h-20 w-20 md:h-24 md:w-24 rounded-2xl bg-gradient-to-br from-emerald-400 via-emerald-600 to-emerald-900 border-2 border-emerald-300 shadow-[0_0_30px_rgba(16,185,129,0.6)] flex items-center justify-center relative overflow-hidden transform hover:scale-105 transition-all">
                    <div class="absolute inset-0 bg-gradient-to-tr from-transparent via-white/30 to-white/50 opacity-40"></div>
                    <span class="text-3xl md:text-4xl font-display font-black text-white drop-shadow-[0_3px_6px_rgba(0,0,0,0.7)] relative z-10">
                      {{ subRoll }}
                    </span>
                  </div>
                }
            ...
            <div class="flex items-center gap-2 z-20">
              <div class="bg-black text-emerald-400 text-xs md:text-sm font-black px-4 py-1.5 rounded-lg border border-emerald-500 shadow-2xl uppercase tracking-widest">
                @if (roll.rolls.length > 1 && roll.mode === 'normal') {
                  {{ roll.rolls.length }}{{ roll.dieType }}
                } @else {
                  {{ roll.dieType }}
              ...
              @if (roll.rolls.length > 1 && roll.mode === 'normal' && roll.dieType !== 'd20') {
                <div class="bg-emerald-500 text-slate-950 text-xs md:text-sm font-black px-4 py-1.5 rounded-lg border border-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.5)] uppercase tracking-widest animate-pulse">
                  Total: {{ roll.total }}
                </div>
              }
            </div>
